...
How to exit: run the command exit
*/
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
#define HISTORY_FILE "nadella_command.history" // History file name
...
void load_history() {
    ...
    while ((bytes_read = read(fd, &buffer[buffer_index], 1)) > 0) {
        if (buffer[buffer_index] == '\n') {
            ...
            add_to_history(buffer); // Add the command to history
...
void save_history() {
    ...
    // Check if the last command is 'exit' and exclude it
    if (history_count > 0 && strncmp(history[(history_count - 1) % HISTORY_SIZE], "exit", 4) == 0) {
        end--;
    }
    ...
    for (int i = start; i < end; i++) {
        int index = i % HISTORY_SIZE;
        if (write(fd, history[index], strlen(history[index])) < 0 ||
            write(fd, "\n", 1) < 0) {
...
// Handle exit by saving history and exiting
void handle_exit() {
    save_history();
    exit(0);
}

// Setup function to read user input and prepare arguments
void setup(char inputBuffer[], char *args[], int *background) {
    ...
    start = -1;
    if (length == 0)
        handle_exit(); // ^d was entered, end of user command stream
    if (length < 0) {
        perror("error reading the command");
        handle_exit(); // terminate with error code of -1
    }
    ...
    if (strncmp(inputBuffer, "exit", 4) == 0 && (inputBuffer[4] == '\n' || inputBuffer[4] == '\0')) {
        handle_exit(); // Save history and exit
    }
